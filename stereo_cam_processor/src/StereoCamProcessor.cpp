...
void StereoCamProcessor::setBlockMatchParams(bool useBM, int preset, int minDisp, int numDisp, int sadWindowSize, bool dispSmoothnessAuto, int dispSmoothness1,
	...
	if (dispSmoothnessAuto) {
		this->dispSmoothness1 = 8 * sadWindowSize * sadWindowSize * 2 * 2;
		this->dispSmoothness2 = 32 * sadWindowSize * sadWindowSize * 2 * 2;
	}

	constructBlockMatch();
	calculateDisparity(true);
...
void StereoCamProcessor::calculateDisparity(bool override) {

	if (override && (leftImage.empty() || rightImage.empty()))
		return;

	if (!override && (!leftImageReceived || !rightImageReceived))
		return;
	...
	ROS_INFO("MinMax Disp Values: %f %f", minVal, maxVal);
	...
	cv::minMaxLoc(dispImageGS, &minVal, &maxVal);

	ROS_INFO("MinMax GS Disp Values: %f %f", minVal, maxVal);
...
void StereoCamProcessor::runBlockMatch	() {
	if (useBM)
		...
		stereoSGBM(leftImageGSRemap, rightImageGSRemap, dispImageTemp);
		dispImageTemp.convertTo(dispImage, CV_32F, (1./16.));
		ROS_INFO("Temp type: %i Disp Type: %i Empty %i", dispImageTemp.type(), dispImage.type(), dispImage.empty());
	}
}
...
//	cv::reprojectImageTo3D(dispImage, threeDImage, q, 0, CV_32F);
	...
	store << "dispImageTemp" << dispImageTemp;
	...
	for (int i = 0; i < 4; i++)
		for (int y = 0; y < 4; y++)
			ROS_INFO("Q(%i)(%i): %f", i, y, q.at<float>(i, y));
	...
	*/
	...
	ROS_INFO("reproject to 3d min %f max %f", min, max);
...
void StereoCamProcessor::convertToPointCloud() {
	...
	for (size_t v = 0; v < cloud.height; v++)
	{
		for (size_t u = 0; u < cloud.width; u++)
		{
			...
			const cv::Vec3b & colorp = color.at<cv::Vec3b> (v, u);
